...
function DSpaceSetupAutocomplete(formID, args)
{
    ...
    var options = {
    		...
    		search: function(event, ui) {
    			jQuery("#"+args.inputName+"_details").html('');
    		...
    		select: function(event, ui){
				...
				updateAuthorityDetailsInfo(args.contextPath, args.metadataField, args.inputName, authValue);
				// make indicator blank if no authority value
                DSpaceUpdateConfidence(document, args.confidenceIndicatorID,
                    authValue == null || authValue == '' ? 'blank' :'accepted');
			}			
    };
    var inputAuto = input.autocomplete(options);
    inputAuto.data("ui-autocomplete")._renderItem = function (ul, item) {
	         return jQuery("<li></li>")
	             .data("item.autocomplete", item)
	             .append("<a>" + item.label + "</a>")
	             .appendTo(ul);
	     };
	inputAuto.change(function(evt){
		...
		if (newauth != lastSel.authority || newval != lastSel.value)
		{
			...
			{
				authInput.val(' ');
				updateAuthorityDetailsInfo(args.contextPath, args.metadataField, args.inputName, '');
				DSpaceUpdateConfidence(document, args.confidenceIndicatorID, 'blank');
			}	
		}
	});
	
	inputAuto.on("input", function(evt){
...
function updateAuthorityDetailsInfo(contextPath, metadataField, inputName, authValue)
{
	if (authValue != null && authValue != '')
	{
		jQuery.ajax({url:  contextPath+"/json/authorityDetails",
			data: {
				field: metadataField,
				key: authValue
			},
			success: function(data){
				var templateId = "tmpl-authority-details-"+data.authorityName;
				if(jQuery("#"+templateId).length == 0) {
					templateId = "tmpl-authority-details-ERR_NOT_FOUND";
				}
				jQuery("#"+inputName+"_details").html(tmpl(templateId, data.details));
			}
		});
	}
	else
	{
		jQuery("#"+inputName+"_details").html('');
	}
}
...
function DSpaceUpdateConfidence(doc, confIndicatorID, newValue)
{
	...
	if (newValue == 'accepted')
		confElt.className = "form-control-feedback inline-feedback fa fa-check text-success";
    else
    	confElt.className = "";
}
...
function showAuthorityDetails(elementID, authorityName, data){
	jQuery("#"+elementID).html(tmpl("tmpl-authority-details-"+authorityName, data));
}