...
var quickSearch = function(){
    ...
    $.widget( "custom.catcomplete", $.ui.autocomplete, {
        ...
        _renderItem:function (ul, item) {
            if ((window.location.hash == '#citation' || $('.advanced-search__top-section .tab__nav .active').hasClass('citation-search__tab')) && $(':focus').attr('name') !== 'AllField') {
                $(ul).attr('data-type', 'citation');
            } else {
                $(ul).attr('data-type', '');
            }
        ...
        initAutoComplete: function (dropOption) {
            ...
            $('.quickSearchFormContainer .autocomplete').catcomplete({
                source: function (request, response) {
                    ...
                    if (window.location.hash == '#citation' || $('.advanced-search__top-section .rlist tab__nav .active').hasClass('citation-search__tab')) {
                        var selectedOption = 'Citation';
                    } else {
                        var selectedOption = $(".quickSearchFormContainer .dropblock--tab .active a").data('searchIn');
                    }
...
function sendAjaxRequest($inputSearchText,dropOption,enteredTerm ,autoCompleteSearchType,selectedOption ,response){
    ...
    var confNumOfTopics = $inputSearchText.data('topics-conf');
    var confNumOfContributors = $inputSearchText.data('contributors-conf');
    var seriesKey = $('input[name=SeriesKey]', '[name=thisJournalQuickSearch]').val();
    ...
    if (dropOption === 'citation'){ //quick fix for LIT-138406
    ...
    } else if (selectedOption === 'thisJournal') {
        var autoCompeteSearchUrl = ['/action/doSuggest?target=', autoCompleteSearchType, 'auto-complete&query=', enteredTerm,
            ...
            '&cs=', confNumOfContributors, '&fl=PubID', '&SeriesKey=', seriesKey].join('');
    } else{
        var autoCompeteSearchUrl = ['/action/doSuggest?target=', autoCompleteSearchType, 'auto-complete&query=', enteredTerm,
            '&hs=', confNumOfHistoryItems, '&pts=', confNumOfPublicationTitles, '&ts=', confNumOfTopics,
        ...
        .done(function (resultData) {
            ...
            var getSuggestion = function(item){
                ...
                if (item.param === 'history') {
                    ...
                    suggestion['value'] =  ['/action/doSearch?target=default&', item.param, '=', item.value, selectedOption === 'thisJournal' ? '&SeriesKey=' + seriesKey : ''].join('');
                }
                return suggestion;
            };
            ...
            $.each(resultData, function (i, item) {
                    if (i == 0 && $inputSearchText.attr('name')!='quickLinkJournal') {
                        var x = {
                            ...
                            'url': '/action/doSearch?AllField=' + encodeURIComponent(enteredTerm) + (selectedOption === 'thisJournal' ? '&publication=' + seriesKey : ''),
                            'value': '/action/doSearch?AllField=' + encodeURIComponent(enteredTerm)
                        };
...
// }
;
function loadRecaptcha(){if(typeof grecaptcha==='undefined')return;$('.g-recaptcha').filter(function(){return!$(this).hasClass('explicit');}).each(function(){grecaptcha.render($(this)[0],$(this).data());});}
...
if((e.keyCode||e.which)===tabKey){if(!revers){tabEvent();}else{tabRevers();}}});$('.show-login').click(function(event){$popups.addClass('hidden');$popup.removeClass('hidden');$popup.attr('aria-hidden',false);$popup.find('#username').focus();$('body, html').addClass('noscroll');event.preventDefault();});$('.access-denial-show-login').click(function(event){$popups.addClass('hidden');$accessPopup.removeClass('hidden');$accessPopup.find('#login').focus();$('body, html').addClass('noscroll');event.preventDefault();});$popup.on('click','.close',function(e){e.preventDefault();$('body, html').removeClass('noscroll');$popup.attr('aria-hidden',true);$popup.addClass('hidden');$eye.addClass('hidden icon-eye').removeClass('icon-eye-blocked');$submit.attr('disabled',true);$remember.attr('checked',false);$login.val('');$password.val('');$message.html('');});$popup.on('keyup',function(e){if(e.keyCode===27){$popup.find('.close').trigger("click");$popup.attr('aria-hidden',true);}
...
$content.find(".LBD_CaptchaDiv").find(".LBD_ReloadLink").trigger("click");$('.request-reset-password-form').trigger("reset");}).fail(function(){$original.removeClass('hidden');$success.addClass('hidden');$message.html('Unknown error');});});});
