...
(function (ns, $) {
    ...
    ns.trackArticlePageEvents = function () {
        ...
        // article history
        $doc.on('click', '.ww-citation-history-wrap', function () {
...
(function (ns, $) {
    ...
    SCM.SplitScreen.init = function () {
        ...
        (function () {
            ...
            SCM.SplitScreen.toggleSplitView = function (override) {
                ...
                if (!splitView) {
                    var elementsToAddId = 'p,section.abstract,blockquote,div.block-child-p';
    ...
    SCM.SplitScreen.WidgetStore.register({
        ...
        render: function (ArticleFulltext, ArticleTopInfo, ArticleLinks, Toolbar, ArticleDataSupplements) {
            ...
            var historys = getElementsByClassList(ArticleTopInfo, ['ww-citation-history-wrap', 'ww-history-close']);
       
            for (var i = 0; i < historys.length; i++) {
                historys[i].addEventListener('click', function (e) {
                    var $pubHistoryDetails = $('.ww-history').last();
                    $pubHistoryDetails.slideToggle();
                });
            }
...
SCM.JournalCitationFilter = (function ($, undefined) {
    ...
    function _initializeHiddenFields() {
        placeholderTextJournal = document.getElementById('Config_PlaceholderTextJournal').value || '';
        placeholderTextYear = document.getElementById('Config_PlaceholderTextYear').value || '';
        placeholderTextVolume = document.getElementById('Config_PlaceholderTextVolume').value || '';
        placeholderTextIssueNo = document.getElementById('Config_PlaceholderTextIssueNo').value || '';
        placeholderTextStartPage = document.getElementById('Config_PlaceholderTextStartPage').value || '';
        validationMessageJournal = document.getElementById('Config_ValidationMessageJournal').value || '';
        validationMessageYear = document.getElementById('Config_ValidationMessageYear').value || '';
        validationMessageVolume = document.getElementById('Config_ValidationMessageVolume').value || '';
        validationMessageIssueNo = document.getElementById('Config_ValidationMessageIssueNo').value || '';
        validationMessageStartPage = document.getElementById('Config_ValidationMessageStartPage').value || '';
    }
...
$(document).ready(function (event) {
    if ($('#hfJournalCitationFilterLoaded').val() !== "true") {
        return;
    }
				...
				for ( dRow = 0; dRow < numRows; dRow++ ) { // create date picker rows
					...
					for ( dow = 0; dow < 7; dow++ ) { // create date picker days
						...
						tbody += "<td class='" +
							...
							"' href='#' aria-label='" + printDate.getDate() + "'>" + printDate.getDate() + "</a>" ) ) + "</td>"; // display selectable date
...
(function (ns, $) {
    ...
    ns.initDatepicker = function () {

        //remove datepicker trigger if already exists. It will be added later in this script
        $('.ui-datepicker-trigger').remove();

        var $datepickerInput = $(".datepicker__input"),
        ...
        $datepickerInput.datepicker({
            ...
            onClose: function () {
                ...
                if ($activeTrigger) {
                    ...
                    // set focus to closest datepickerTrigger
                    $(this).siblings('.ui-datepicker-trigger').focus();
                }
            }
        }).keydown(function (e) {
            ...
            if (code !== 9 || code !== 13) {
                ...
                if (code === 37 || code === 38 || code === 39 || code === 40) {
                    ...
                    // something other than TAB, ENTER or ARROWS was pressed
                    return false;
                }
            } 
        });

        var $uiDatepickerTrigger = $('.ui-datepicker-trigger'), // these buttons are created in function above 
            $activeTrigger;

        $datepickerInput.attr('tabindex', -1); // take input out of tab order
        $uiDatepickerTrigger.attr('tabindex', 0); // add button to tab order

        // give each datepickerTrigger an ARIA label
        $uiDatepickerTrigger.each(function () {
            var $thisButton = $(this),
                $siblingInputID = $thisButton.siblings('input').attr('id');

            $thisButton.attr('aria-label', 'open ' + $siblingInputID + ' datepicker, use arrows to select date');
        });

        // the datepickerTrigger to be focused on when datepicker closes
        $uiDatepickerTrigger.keypress(function () {
            $activeTrigger = $(this);
        });
    ...
    */
    ...
    function updateArticleLeftNav() {
        ...
        if (lastScrollPosY >= $header.height()
            ...
            if (currPage === "article" || currPage === "search") {
                $leftNav.css('top', '0px');
...
(function (ns, $, accessIcons) {
    ...
    **********************************************************************/
    ...
    ns.togglePubHistory = function () {
        var $pubHistoryDetails = $('.ww-history');

        $('.ww-citation-history-wrap').on('click', function () {
            $pubHistoryDetails.slideToggle();
        });

        $('.ww-history-close').on('click', function () {
            $pubHistoryDetails.slideToggle();
        });
    };
    ...
    // ----
    ns.initDatePicker = function () {
        if ($('.pg_advancedsearch').length && $('#dpStartDate').length) {
            $('#dpStartDate, #dpEndDate, #dpStartDateAuthor, #dpEndDateAuthor').fdatepicker();
        }
    };
...
$(function () {
    ...
    function bindKeyToDate(dateToBind, update) {
        dateToBind.bind("keydown", function (e) {
            // http://stackoverflow.com/questions/15235499/jquery-ui-datepicker-prevent-enter-selecting-when-readonly
            if (e.which === 13) {
                var dateValue = dateToBind.val();
                if (isYearOnly(dateValue)) {
                    updateYearOnlyToFullDate(dateToBind, update);
                    e.preventDefault();
    ...
    (function() {
        ...
        //TODO: Consolidate Date functions from (AdvancedSearch.JS, AdvancedFeatureSearch.JS, and SolrSearch.CSHTML) into a new DateHelper.JS file
        ...
        var isValidSolrDate = function(date) {

            //Allow the following formats: MM/DD/YYYY; MM/DD/YY; M/D/YYYY; M/D/YY
            var matches = /^(\d{1,2})[-\/](\d{1,2})[-\/](\d{2,4})$/.exec(date);
            ...
            //if year is 2 digits, prefix it with current century(20) because part of the validation is based on getFullYear().
            if (y.length == 2) {
                y = '20' + y;
            }
        ...
        var isYearOnly = function(date) {
            var matches = /^(\d{4})$/.exec(date);

            return (matches && (matches[1] > -1));
        }

        var updateYearOnlyToFullDate = function(dateTxtField, isStartDate) {
        ...
        //Formats a given Date to mm/dd/yyyy format.
        var formatDate = function(dateValue) {

            //Allow the following formats: MM/DD/YYYY; MM/DD/YY; M/D/YYYY; M/D/YY
            var matches = /^(\d{1,2})[-\/](\d{1,2})[-\/](\d{2,4})$/.exec(dateValue);
            ...
            //Date: Prefix 0 if value contains only 1 digit.
            if (d.length == 1) {
                fmtD = '0' + d;
            }
            //Month: Prefix 0 if value contains only 1 digit.
            if (m.length == 1) {
                fmtM = '0' + m;
            }

            //Construct Date
            var composedDate = fmtM + "/" + fmtD + "/" + fmtY;
        ...
        $updateTarget.on('click', 'a.close', function(e) {
            ...
            bindKeyToDate($("#fromDate"), true);
            bindKeyToDate($("#ToDate"), false);

            if (!$(".ui-datepicker-trigger") || $(".ui-datepicker-trigger").length === 0) {
                if ($(".datepicker__input").hasClass('hasDatepicker')) {
                    $(".datepicker__input").removeClass('hasDatepicker');
                }
                SCM.AccessibleDatepicker.initDatepicker();
            }
        ...
        $updateTarget.on('click', '#btnRangeSearch', function(e) {
            ...
            if (isYearOnly(fromDate)) {
                fromDate = updateYearOnlyToFullDate($('#fromDate'), true);
            }

            var toDate = $('.date-range-input-radio[value=single]').is(':checked') ? fromDate : $('#ToDate').val();

            if (toDate !== '' && isYearOnly(toDate)) {
                toDate = updateYearOnlyToFullDate($('#ToDate'), false);
            }

            //Validate Dates
            if (!isValidSolrDate(fromDate) || !isValidSolrDate(toDate)) {
                alert("Please enter valid dates");
                return;
            }

            //Format dates to MM/DD/YYYY format before comparing dates and passign them to query
            var formattedStartDate = formatDate(fromDate);
            var formattedEndDate = formatDate(toDate);
            ...
            var rgurl = $(this).attr('data-url').replace('queryText', formattedStartDate + ' TO ' + formattedEndDate);
            SCM.SearchResults.doSolrSearch({ queryString: rgurl });
        });
    ...
    SCM.SearchResults = $.extend(SCM.SearchResults, function ($, undefined) {
        ...
        function bindObjectsOnUpdate(myobject, $updateTarget) {
            ...
            SCM.AccessibleDatepicker.initDatepicker(); // RE-init Datepicker if it hasn't already

            scrollIfNotInView($updateTarget[0]);

            // Bind again after ajax finishes; can't use variables from outside ajax block, hence $("#fromDate") instead of $fromDate
            bindKeyToDate($("#fromDate"), true);
            bindKeyToDate($("#ToDate"), false);
...
(function (SolrSearch, $, undefined) {
    ...
    function initDateRangeToggle() {
        $('.date-range-input-radio')
            .on('click',
                function () {
                    if ($(this).attr('value') === 'single') {
                        $('.date-input-to').hide();
                        $('.date-label-from span').text("Search on this date").append("<br/>");
                    } else {
                        $('.date-input-to').show();
        ...
        bodyVar.on('click',
            ...
            function (e) {
                ...
                var fullUrl = baseUrl + "?page=1" + freeSearchQueryElement + siteIdElement +
                              searchSourceTypeElement + allJournalsElement;
...
(function(iou, $, ls, undefined) {
    ...
    iou.replaceOpenUrlPlaceHolders = function (openUrlPlaceHolderElements, replaceLinkHtml) {
        if (replaceLinkHtml !== null && replaceLinkHtml !== '') {
    ...
    iou.getAndReplaceOpenUrlPlaceHolders = function (openUrlPlaceHolderElements) {
        ...
        $.ajax({
            ...
            success: function (resultObject) {
                if (resultObject !== null && resultObject !== 'undefined') {
                    ...
                    // TargetUrl , DisplayOpenUrlLink , HasOpenUrlImage, LinkImageSource ,LinkImageText 
                    var replacelink = '';
    ...
    iou.checkAndReplaceOpenUrlPlaceHolders = function () {
        ...
        if (typeof window.App.LoginUserInfo !== 'undefined' && window.App.LoginUserInfo.isInstLoggedIn > 0) {
            // check if article has any openurl templates
            var openUrlPlaceHolders = $('span[class="inst-open-url-holders"]');
            if (openUrlPlaceHolders !== null && openUrlPlaceHolders !== undefined && openUrlPlaceHolders.length > 0) {
                ...
                if (ls !== null && window.sessionStorage !== null && typeof window.App.LoginUserInfo.currentSessionId !== 'undefined') {
                    ...
                    if (storedLink !== null) {
                        iou.replaceOpenUrlPlaceHolders(openUrlPlaceHolders, storedLink);
                    } else {
                        iou.getAndReplaceOpenUrlPlaceHolders(openUrlPlaceHolders);
                    }
                }
                else {
                    iou.getAndReplaceOpenUrlPlaceHolders(openUrlPlaceHolders);
...
(function (ns, headings, $) {
   ...
   SCM.BookToc.Headings = SCM.BookToc.Headings || {},
   jQuery);
