...
(function (ns, $) {
    ...
    ns.handleUnresolvedRelatedObjectLinks = function () {         
        $("a[isbn]").attr("href", "/CustomError?statusCode=404"); 
    };
...
$(function () {
    ...
    $(document).on('click', '.facetShowMore', function (event) {
        if ($(this).closest('.sf-facet-list').children('.sf-facet:gt(11):visible').length) {
            $(this).closest('.sf-facet-list').children('.sf-facet:gt(11):visible').animate({ height: 'toggle' }, 400);
            $(this).text('Show more');
        }
        else {
            $(this).closest('.sf-facet-list').children('.sf-facet').not(':visible').animate({ height: 'toggle' }, 400);
            $(this).text('Show less');
        }
        event.preventDefault();
    });
    ...
    function collapseFacets() {
        $('.collapsible-facet .sf-facet-list').each(function () {
            if ($(this).children('.sf-facet').length > 12) {
                $(this).children('.facetShowMoreLi').remove();
                $(this).children('.sf-facet:gt(11)').hide();
...
(function (ns, $) {
    ...
    ns.queryBuilderHandler = function () {
        ...
        var paramsToRemove = ['fd_IsLatestEdition', 'cqb', 'qb', 'page'];

        queryParams = queryParams.filter(function (el) {
            return !paramsToRemove.indexOf(el) < 0;
        });
