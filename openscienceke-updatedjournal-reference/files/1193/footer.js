    ...
    $('.js-main, .js-querybuilder-wrap').off('click').on('click', '.querybuilder-submit', function () {
        ...
        $('.querybuilder-wrap .querybuilder-instance').each(function (i) {
            ...
            if (instanceVisible) {
                ...
                if (instanceIndex === 1) {
                    var searchTerm = encodeURIComponent($this.find('#instance_QueryBuilderSearchTerm').val().trim());
                ...
                switch (qbSearchTypeDisplayName) {
                    ...
                    case "All":
                        ...
                        if (qbSearchTerm !== "") {
                            qbSearchTerm = encodeURIComponent(qbSearchTerm);
                            currentDataUrl = updateDataUrlForAllOption(qbSearchTerm, currentDataUrl);
    ...
    */
    ...
    function init() {
        ...
        if ($body.hasClass('pg_chapterstandard')) {
            ...
            $(window).on('scroll', handleScroll);
            //NARN-13947, removed due to highlighting the very first "dropdown arrow"/.isSelected() item from BookTocSlim.cshtml
            //$('#InfoColumn i.icon-general-arrow-filled-down:first').closest('.toggleArea.expandable').find('.js-tocLink').addClass('active');
        ...
        $(window).on('scroll', function () {
            ...
            if ($('.book-chapter-body').length) {
                if (!$jsJumpLinkTargetIds.length) {
                    $('#ContentColumn h2, #ContentColumn h3').each(function () {
...
(function (ns, $, accessIcons) {
    ...
    **********************************************************************/
    ...
    ns.appendTotalSearchCount = function () {
        var $totalSearchCount = $('#hdnTotalSearchCount');

        // NTPT_PGEXTRA is a javascript variable set in Themes\Client\Views\Scripts\ClientCustom.cshtml.
        // It is used by scholarlyiq to pass parameters via NetInsights.
        // See also: IocImplementations\NetInsightTrackingService.cs
        if (NTPT_PGEXTRA && $totalSearchCount.length) {
            NTPT_PGEXTRA += "&num_results=" + $totalSearchCount.val();
        }
    }
...
(function (SolrSearch, $, undefined) {
    ...
    $('body').on('click', '.js-taxonomy-label', function () {
        ...
        if (!taxQsPresent) {
            ...
            var taxQueryTerm = $.deparam.querystring()["tax"];
            if (taxQueryTerm && taxQueryTerm !== "") {
                ...
                if (taxQueryTerm.indexOf("=" + rootNodeId) > -1) {
                    ...
                    currentDataUrl = currentDataUrl.replace("tax=" + taxQueryTerm, "tax=" + curTaxonomyNodeId);
                ...
                if (resultObject !== null && resultObject !== 'undefined') {
                    ...
                    if (resultObject.DisplayOpenUrlLink == true) {
                        if (resultObject.LinkImageSource !== null && resultObject.LinkImageSource !== '') {
                            replacelink = " <a class='openInAnotherWindow' target='_blank' href='" + resultObject.TargetUrl + "'><img src='" + resultObject.LinkImageSource + "'/></a> ";
